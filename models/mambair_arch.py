...
def test_mambair_denoising():
    # 1. 设置设备
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"正在设备 {device} 上运行测试...")

    # 2. 定义输入参数
    img_size = 64
    in_chans = 3

    # 3. 实例化模型 (针对去噪/保持分辨率配置)
    model = MambaIR(
        img_size=img_size,
        patch_size=1,
        in_chans=in_chans,
        embed_dim=64,  # 嵌入维度
        depths=(6, 6, 6),  # 为了测试速度，减少层数
        d_state=16,

        # --- 关键修改点 ---
        upscale=1,  # 放大倍率为 1
        upsampler=''  # 为空时，触发去噪逻辑 (denoising branch)
        # ----------------
    ).to(device)
    ...
    print(f"==============================")
    print(f"模型总参数量: {total_params / 1e6:.4f} M ({total_params} 个)")
    print(f"可训练参数量: {trainable_params / 1e6:.4f} M ({trainable_params} 个)")
    print(f"==============================")
    # --------------------------------

    # 4. 构造随机输入张量 [Batch, Channel, Height, Width]
    # 模拟一张 64x64 的图片
    dummy_input = torch.randn(1, in_chans, img_size, img_size).to(device)
    ...
    with torch.no_grad():
        try:
            ...
            if output.shape == dummy_input.shape:
                print("✅ 测试成功！输出分辨率与输入一致 (保持原分辨率)。")
            else:
                print(f"❌ 尺寸不匹配: 期望 {dummy_input.shape}, 实际 {output.shape}")

        except RuntimeError as e:
            print(f"❌ 发生错误: {e}")
            if "selective_scan" in str(e) and device.type == 'cpu':
                print("提示: 这里的 Mamba 核心算子通常需要 GPU 环境。")


# 运行测试
if __name__ == "__main__":
    test_mambair_denoising()