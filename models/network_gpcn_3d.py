...
class WDPCNet(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
        ...
        num_out_ch = in_chans
        num_feat = 64
        embed_dim = 90 # 这个参数在 __init__ 中被覆盖了, 使用传入的 embed_dim
        ...
        self.conv_first = nn.Conv2d(num_in_ch*3, embed_dim, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        # 频域分支 (PALH) 只需要基于当前的目标层 (x_mid) 进行初始化和校正
        xt = torch.cat((x_mid, torch.zeros_like(x_mid)), dim=1)
        ...
        # 定义一个变量，用于在多级迭代中保存当前最新优化的中间层
        curr_x_mid = x_mid

        for i in range(self.num_layers):
            # 3. 动态拼接：每次提取特征前，都用固定的上下层上下文包裹当前最新优化的中间层
            x_in = torch.cat([x_top, curr_x_mid, x_bottom], dim=1)

            x_first = self.conv_first(x_in)
            # forward_features 第二个参数原本是原始输入，但没用到。为了严谨传 x_first
            res = self.conv_after_body(self.forward_features(x_first, x_first)) + x_first

            # 4. 残差更新：1通道输出 + 1通道当前中间层
            curr_x_mid = self.conv_last(res) + curr_x_mid

            x2 = self.PMLH(b)
            curr_x_mid_cpx = torch.cat((curr_x_mid, torch.zeros_like(curr_x_mid)), dim=1) + x2

            b = curr_x_mid_cpx.permute(0, 2, 3, 1).contiguous()
            b = torch.view_as_complex(b)
            b = torch.fft.fftshift(torch.fft.fftn(b, dim=(-2, -1), norm='ortho'), dim=(-2, -1))
            b = torch.view_as_real(b)
            b = b.permute(0, 3, 1, 2).contiguous()
            ...
            if (i != self.num_layers - 1):
                curr_x_mid = torch.sqrt(curr_x_mid_cpx[:, 0, :, :] ** 2 + curr_x_mid_cpx[:, 1, :, :] ** 2).unsqueeze(1)

        return curr_x_mid[:, :, :H * self.upscale, :W * self.upscale]
