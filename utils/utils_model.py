...
def fft2(tensor):
    zero = tensor
    tensor = torch.cat((tensor,zero),dim=-1)
    assert tensor.shape[-1] == 2
    # print(data1.shape)
    # print(tensor.shape)
    # tensor = tensor.contiguous()
    data1 = torch.view_as_complex(tensor)
    data1 = torch.fft.fftn(data1, dim=(-2, -1), norm='ortho')
    data1 = torch.view_as_real(data1)
    data1 = fftshift(data1, axes=(-3, -2))
    return data1

def ifft2(data1):
    assert data1.shape[-1] == 2
    data1 = ifftshift(data1, axes=(-3, -2))
    data1 = torch.view_as_complex(data1)
    data1 = torch.fft.ifftn(data1, dim=(-2, -1), norm='ortho')
    # data1 = torch.view_as_real(data1)
    data1 = torch.abs(data1)
